...
class ServiceName:
    ...
    # Known AWS service patterns - Domain Knowledge
    _AWS_SERVICE_PATTERNS: Set[str] = {
    ...
    def is_known_aws_service(self) -> bool:
        ...
        """
        # Check exact match
        if self.name in self._AWS_SERVICE_PATTERNS:
            return True
        
        # Check if it starts with known patterns (for services with suffixes)
        for pattern in self._AWS_SERVICE_PATTERNS:
    ...
    def get_abbreviation(self) -> str:
        ...
        for abbrev, full_name in self._SERVICE_ABBREVIATIONS.items():
    ...
    def from_abbreviation(cls, abbreviation: str) -> 'ServiceName':
        ...
        Complexidade: O(1)
        """
        full_name = cls._SERVICE_ABBREVIATIONS.get(abbreviation.upper())
        if full_name:
